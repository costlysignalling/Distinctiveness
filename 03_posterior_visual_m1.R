...
d$nat<-as.factor(as.character(d$nat))
levels(d$nat)
summary(d$nat)

d$nat<-factor(d$nat,levels = c("NAM","CMR","TR","RO","CZ","UK","COL","BRAZ","IND","VIE"))
summary(as.factor(d$nat))

#Create labels for varying intercepts and slopes
labs<-paste(rep(labnames,each=dim(post$pars)[2],times=2),rep(c("F","M"),each=length(labnames)*dim(post$pars)[2]),rep(levels(d$nat),times=length(labnames)*2),sep="_")
labs
...
postsn<-lapply(levels(d$nat),function(n){nwise[,labnat==n][,re.ar]})
str(postsn)
...
plotMi(postsn[[9]],axis=F,below="Namibia",col.bg=colcont[4])
...
plotMi(postsn[[10]],axis=F,below="Cameroon",col.bg=colcont[4])
...
plotMi(postsn[[7]],axis=F,below="Total",col.bg=colcont[3])
plotMi(posts,axis=T)

plot(NULL,ylim=c(const2,0),xlim=c(0,1),type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",bty="n")
text(0,const2-0.5,"United Kingdom",pos=4,font=2)
plotMi(postsn[[6]],axis=F,below="Brazil",col.bg=colcont[2])
plotMi(postsn[[8]],axis=F,below="Prior (Total)",col.bg=colcont[3])
plotMi(prios,axis=T)
dev.off()

precis(as.data.frame(sigmas))
...
rownames(estimates)<-c(levels(d$nat),"Total")
...
tiff("sigmas_m1.tif",width=12,height=7.5,units="cm",res=600,compression = "lzw")
...
save(posts,postsn,nwise,s_vary,Rho,file="posterior_samples_m1.Rdata")

...
(prec.fit<-precis(mAge,depth=3))

#Extract 10 000 samples for visualization
post<-extract.samples(mAge)
...
#In model comparison with Age and BMI, we cannot use UK and IND sample, where body weight information is missing  at least in one gender
nrow(d.s)
...
#The original extracted prior is rugged. But each sigma is exponential(1) so it is not difficult to do better
str(prios)
for(i in 1:ncol(prios)){
  prios[,i]<-rnorm(nrow(prios),0,0.5)
}
...
#Standard deviation of the likelihood function around the linear model
sigma<-post$sigma
...
all.params<-read.table("params_mAge.txt",sep="\t",header = T)
...
tiff("posterior_mAge.tif",width=20,height=20,units="cm",res=600,compression = "lzw")
...
for(i in 1:3){
  ...
  text(0,max(dei$y)+const1+const2-0.3,ifelse(i==3,"","Attractiveness ~"),pos=4,font=2,xpd=T)
}
...
write.table(estimates,"Parameter_estimates_mAge.txt",sep="\t",col.names=NA)

#Probability that parameters are positive and prob. that they are abs. larger than 0.1
...
rownames(pro.pos.big)<-c(levels(d.s2$nat),"Total")

write.table(pro.pos.big,"Parameter_probabilities_mAge.txt",sep="\t",col.names=NA)
...
colnames(tab.deltas)<-c("a[women]-a[men]","bST[women]-bST[men]","bAs[women]-bAs[men]","bD[women]-bD[men]","bH[women]-bH[men]","bBMI[women]-bBMI[men]","bAge[women]-bAge[men]")
rownames(tab.deltas)<-c(levels(d.s2$nat),"Total")

write.table(tab.deltas,"deltas_sex_mAge.txt",sep="\t",col.names=NA)


#Sigma summary
s_vary<-s_vary[,c(rep(c(0,7),times=(ncol(s_vary)-1)/2)+rep(1:((ncol(s_vary)-1)/2),each=2),length(s_vary))]
s_vary_prior<-s_vary_prior[,c(rep(c(0,7),times=(ncol(s_vary)-1)/2)+rep(1:((ncol(s_vary)-1)/2),each=2),length(s_vary))]
...
all.params<-read.table("params_sigmas_mAge.txt",sep="\t",header = T)
...
tiff("sigmas_mAge.tif",width=12,height=9.5,units="cm",res=600,compression = "lzw")
...
write.table(sig.tab,"Sigma_estimates_mAge.txt",sep="\t",col.names=NA)
...
save(posts,postsn,nwise,s_vary,Rho,file="posterior_samples_mAge.Rdata")


#Some more summary stats
table(as.integer(d.s3$nat),d.s3$nat)
sum(table(as.integer(d.s3$nat),d.s3$nat))
table(as.integer(d.s3$sex),d$sex)
1097/8

mean(summary(d.s3$nat))
sd(summary(d.s3$nat))

table(as.integer(d.s3$nat[d.s3$sex=="F"]),d.s3$nat[d.s3$sex=="F"])
table(as.integer(d.s3$nat[d.s3$sex=="M"]),d.s3$nat[d.s3$sex=="M"])

mean(summary(d.s3$nat[d.s3$sex=="F"]))
sd(summary(d.s3$nat[d.s3$sex=="F"]))

mean(summary(d.s3$nat[d.s3$sex=="M"]))
sd(summary(d.s3$nat[d.s3$sex=="M"]))

...
#I do not do these steps for d.s3 (they are too similar)
...
d.list3<-list(Attractiveness=d.s3$Attractiveness,
              Sextypicality=d.s3$Sextypicality,
              Asymmetry=d.s3$Asymmetry,
              Distinctiveness=d.s3$Distinctiveness,
              Height=d.s3$Height,
              BMI=d.s3$BMI,
              Age=d.s3$Age,
              sex=as.integer(d.s3$sex),
              nat=as.integer(d.s3$nat))
...
#Basic model with the subset of the data that include info on body height, BMI, and Age
m1.Age<-ulam(
  alist(
    Attractiveness ~ normal(mu, sigma),
    ...
    Rho ~ lkj_corr(2)
  ), data=d.list3 , chains=10 , cores=10, log_lik=TRUE ,iter = 8000, control=list(max_depth=18,adapt_delta=0.99))
save(m1.Age,file="m1.Age.Rdata")
...
mBMI.Age<-ulam(
  alist(
    ...
    Rho ~ lkj_corr(2)
  ), data=d.list3 , chains=10 , cores=10, log_lik=TRUE ,iter = 8000, control=list(max_depth=18,adapt_delta=0.992))
save(mBMI.Age,file="mBMI.Age.Rdata")

#Model including body height, BMI, and Age
mAge<-ulam(
  alist(
    Attractiveness ~ normal(mu, sigma),
    
    mu <- pars[nat,(sex-1)*7+1]+pars[nat,(sex-1)*7+2]*Sextypicality+pars[nat,(sex-1)*7+3]*Asymmetry+pars[nat,(sex-1)*7+4]*Distinctiveness+pars[nat,(sex-1)*7+5]*Height+pars[nat,(sex-1)*7+6]*BMI+pars[nat,(sex-1)*7+7]*Age,
    
    vector[14]:pars[nat] ~ multi_normal(c(amu[1],bSTmu[1],bAsmu[1],bDmu[1],bHmu[1],bBMImu[1],bAgemu[1],amu[2],bSTmu[2],bAsmu[2],bDmu[2],bHmu[2],bBMImu[2],bAgemu[2]),Rho,sigma_nat),
...
save(mAge,file="mAge.Rdata")

precis(mAge,depth=2)

(comp.tab<-compare(m1.Age,mBMI.Age,mAge))
comp.tab<-cbind(model=row.names(comp.tab),round(comp.tab,2))
write.table(comp.tab,file="WAIC.table.Age.txt",row.names=F,sep="\t")

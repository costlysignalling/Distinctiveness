...
rownames(cor.eff)<-c("Intercept F (a_F)","slope Sextypicality F (bST_F)","slope Asymmetry F (bAs_F)","slope Distinctiveness F (bD_F)","Intercept M (a_M)","slope Sextypicality M (bST_M)","slope Asymmetry M (bAs_M)","slope Distinctiveness M (bD_M)")
...
cor.eff<-matrix(apply(Rho,2,mean,na.rm=T),ncol=sqrt(ncol(Rho)))
rownames(cor.eff)<-c("Intercept F (a_F)","slope Sextypicality F (bST_F)","slope Asymmetry F (bAs_F)","slope Distinctiveness F (bD_F)","slope Height F (bH_F)","slope BMI F (bBMI_F)","Intercept M (a_M)","slope Sextypicality M (bST_M)","slope Asymmetry M (bAs_M)","slope Distinctiveness M (bD_M)","slope Height M (bH_M)","slope BMI M (bBMI_M)")
...
colnames(cor.F)<-c("Attr","Sex T.","Asym","Dist.","Height","BMI")
colnames(cor.M)<-c("Attr","Sex T.","Asym","Dist.","Height","BMI")
rownames(cor.M)<-rep("",nrow(cor.M))


my.mixed<-function (corr, lower = "number", upper = "circle", 
          tl.pos = c("d", "lt", "n"), diag = c("n","l", "u"), bg = "white", addgrid.col = "grey", 
          lower.col = NULL, upper.col = NULL, plotCI = c("n","square", "circle", "rect"), mar = c(0, 0, 0, 0), legend = T,...) 
{
  tl.pos <- match.arg(tl.pos)
  diag <- match.arg(diag)
  n <- nrow(corr)
  adjust_plotCI <- function(plotCI, method) {
    if (plotCI != "rect" || method %in% c("circle", 
                                          "square")) {
      return(plotCI)
    }
    return("n")
  ...
  corrplot(corr, add = TRUE, type = "lower", method = lower, 
           ...
           plotCI = plotCI_lower, col = lower.col, mar = mar, ...)
  if (diag == "n" && tl.pos != "d") {

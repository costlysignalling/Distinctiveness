...
(prec.fit<-precis(mBMI,depth=3))

#Extract 10 000 samples for visualization
post<-extract.samples(mBMI)
...
all.params<-read.table("params_mBMI.txt",sep="\t",header = T)
...
tiff("posterior_mBMI.tif",width=20,height=20,units="cm",res=600,compression = "lzw")
...
write.table(estimates,"Parameter_estimates_mBMI.txt",sep="\t",col.names=NA)
...
colnames(tab.deltas)<-c("a[women]-a[men]","bST[women]-bST[men]","bAs[women]-bAs[men]","bD[women]-bD[men]","bH[women]-bH[men]","bBMI[women]-bBMI[men]")
rownames(tab.deltas)<-c(levels(d.s2$nat),"Total")

write.table(tab.deltas,"deltas_sex_mBMI.txt",sep="\t",col.names=NA)


#Sigma summary
s_vary<-s_vary[,c(rep(c(0,6),times=(ncol(s_vary)-1)/2)+rep(1:((ncol(s_vary)-1)/2),each=2),length(s_vary))]
s_vary_prior<-s_vary_prior[,c(rep(c(0,6),times=(ncol(s_vary)-1)/2)+rep(1:((ncol(s_vary)-1)/2),each=2),length(s_vary))]

write.table(data.frame(nam=names(s_vary)),"params_empty.txt",row.names=F,sep="\t")

all.params<-read.table("params_sigmas_mBMI.txt",sep="\t",header = T)
...
tiff("sigmas_mBMI.tif",width=12,height=9.5,units="cm",res=600,compression = "lzw")
...
write.table(sig.tab,"Sigma_estimates_mBMI.txt",sep="\t",col.names=NA)
...
save(posts,postsn,nwise,s_vary,Rho,file="posterior_samples_mBMI.Rdata")
